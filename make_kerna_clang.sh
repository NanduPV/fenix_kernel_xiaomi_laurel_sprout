...
export LD_LIBRARY_PATH=/home/derflacco/toolchains/proton-clang-20201212/lib/
export USE_CCACHE=1
defconfig=/vendor/fenix_defconfig


PATH="/home/derflacco/toolchains/proton-clang-20201212/bin:$PATH"
export CROSS_COMPILE=aarch64-linux-gnu-
make $defconfig CC=clang O=output/

make -j$(nproc --all)  NM="llvm-nm" OBJCOPY="llvm-objcopy" LD="ld.lld" CC="ccache clang -fcolor-diagnostics -Qunused-arguments" O=output/
...
make_zip()
{
		...
		FINAL_ZIP="Oss_Chasing_Spring-${VERSION}-${DATE}.zip"
        zip -r9 "${FINAL_ZIP}" *
		cp *.zip $OUT
		rm *.zip
                rm -rf kernel
